...
#define XFIFO_SetStatus1(status, reg) (reg | status)
#define XFIFO_SetStatus0(status, reg) (reg & ~status)

#define XFIFO_GetStatus(status, reg) (reg & status)

#define XFIFO_ReadReg(addr, offset) Xil_In32((addr) + (offset))

#define XFIFO_WriteReg(addr, offset, data) Xil_Out32((addr) + (offset), (data))
